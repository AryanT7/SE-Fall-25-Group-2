...
from ..schemas import DriverLoginRequest, Token, AssignedOrderOut, DriverLocationIn
from ..models import User, Order, OrderStatus, DriverLocation
...
def get_assigned_orders(driver_id: int, db: Session = Depends(get_db), current: User = Depends(get_current_user)):
    ...
    if current.role.name != 'DRIVER' and current.role.name != 'ADMIN':
        raise HTTPException(status_code=403, detail="Insufficient role")
    if current.role.name == 'DRIVER' and current.id != driver_id:
